...
Usage
-----
    engine = InferenceEngine.from_pretrained("TinyLlama/TinyLlama-1.1B-Chat-v1.0")
    result = engine.generate("...", max_new_tokens=64, mode=GenerationMode.CHUNK_KV_REUSE)
"""
...
class InferenceEngine:
    ...
    def warm_chunks(self, text: str, position_offset: int = 0) -> int:
        """
    ...
    def _normalize_past_kv(past_key_values) -> PastKVType:
        ...
        Never uses (k, v) tuple-unpacking on the outer container — that is
        what breaks when transformers returns a DynamicCache whose iteration
        yields something other than 2-element tuples.
        ...
        # ── Path 1: DynamicCache (transformers ≥ 4.45) — return as-is so the
        #    model receives a Cache object it can call get_seq_length() on.
        if hasattr(past_key_values, "get_seq_length"):
            return past_key_values

        # ── Path 2: DynamicCache with .to_legacy_cache() (transformers 4.38+)
        if hasattr(past_key_values, "to_legacy_cache"):
            legacy = past_key_values.to_legacy_cache()
            # legacy is supposed to be tuple[tuple[Tensor,Tensor]] but index
            # explicitly to be safe regardless of extra elements per layer
            return tuple((layer[0], layer[1]) for layer in legacy)

        # ── Path 3: already a plain tuple of tuples — index, don't unpack
